...
export default function UtilityEditColor() {
  ...
  const handleSubmit = (event) => {
    ...
    $(".error").remove();
    if (!subName.trim()) {
      $("#subName").after('<div class="error">*Enter the Color</div>');
    } else if (!/^[A-Za-z\s]*$/.test(subName.trim())) {
      $("#subName").after(
        '<div class="error">*Color contains only Alphabetics</div>'
      );
    } else {
      final.push("1");
    }
            ...
            <div className="sign-up-form h-100 position-relative">
              <fieldset>
                <legend className="w-100">
                  <h2 className="text-center">Edit Color</h2>
                </legend>
