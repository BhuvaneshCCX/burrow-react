...
export default function UtilityEditSubCategory() {
  ...
  const handleSubmit = (event) => {
    ...
    $(".error").remove();
    if (!name.trim()) {
      $("#name").after('<div class="error">*Enter the Category</div>');
    } else if (!/^[A-Za-z\s]*$/.test(name.trim())) {
      $("#name").after(
        '<div class="error">*Category contains only Alphabetics</div>'
      );
    } else {
      final.push("1");
    }
    if (!subName.trim()) {
      $("#subName").after('<div class="error">*Enter the Sub Category</div>');
    } else if (!/^[A-Za-z\s]*$/.test(subName.trim())) {
      $("#subName").after(
        '<div class="error">*Sub Category contains only Alphabetics</div>'
      );
    } else {
      final.push("2");
    }
            ...
            <div className="sign-up-form h-100 position-relative">
              <fieldset>
                <legend className="w-100">
                  <h2 className="text-center">Edit Sub Category</h2>
                </legend>
